...
├── phase-9-hostbootstrap-core-integration-and-host-driven-3x3.md
...
- The skeletal `hostbootstrap.dhall` at the repository root declares only the project name, the
  thin Dockerfile, and the per-project resource budget (`resources {cpu, memory, storage}`); it
  is read by `hostbootstrap`'s thin Python bootstrapper. Substrate detection, host-tool
  resolution, `ensure` reconcilers, container / VM lifecycle, and kind resource cordoning are
  owned by the [`hostbootstrap-core`](https://github.com/Tuee22/hostbootstrap) Haskell library.
- The rich project-level Dhall (worker / orchestrator roles + cluster bootstrap) and the
  per-case test Dhall are **generated by the project binary**, which emits its own configuration
  schema; the binary is the substrate-aware seam, not a hand-maintained model file.
...
- `src/Daemon/Cluster/*` (kind cluster setup) and the `chart/` directory carry per-substrate
  variation.
- The project binary extends the `hostbootstrap-core` optparse-applicative command tree; its
  `cluster` subcommands accept a `ClusterProfile` (production vs test) and resolve substrate
  selection from the host the bootstrapper hands it.
...
- **Apple target:** Apple Silicon. The thin Python bootstrapper provisions a per-project Colima
  VM sized to the declared resource budget, ensures the host toolchain, builds the project binary
  natively on the host (a Linux ELF cannot exec on macOS), copies it to `./.build/`, and execs
  it; the host worker runs as a caller-owned foreground process. Direct
  `./.build/<project-binary> ...` commands are equivalent.
- **Linux CPU target:** Linux. The bootstrapper builds the project binary inside the thin project
  container (`FROM` the hostbootstrap base tag), copies the ELF out to `./.build/` (same glibc
  family), and runs it on the host; kind node cordoning enforces the resource budget.
- **Linux GPU target:** Linux, built from the CUDA-flavored hostbootstrap base, otherwise the
  same build-in-container / copy-out / run-on-host shape as Linux CPU.
...
- Sprint development and first validation should be possible on the machine that owns the changed
  ...
  supported physical machine, driving a fresh `dst-test-`-prefixed cluster per case under
  `.test_data/` so production `.data/` is never touched. Use `--force-target` to exercise the
  three declared hostbootstrap targets when validating substrate detection, base-image
  selection, or the recursive hostbootstrap invocation per case.
