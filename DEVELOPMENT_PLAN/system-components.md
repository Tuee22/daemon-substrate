...
| `Daemon.Cluster.Profile` (`ClusterProfile`) | 9 (Sprint 9.3) | no (Blocked) |
| `Daemon.Test.Config.Schema` / `Daemon.Test.Config.Render` (binary-generated Dhall) | 9 (Sprint 9.2) | no (Blocked) |
| `Daemon.Test.Integration.Runner` / `Daemon.Test.Integration.Assertions` | 9 (Sprints 9.4–9.5) | no (Blocked) |
...
## Target architecture: hostbootstrap-core inversion (Phase 9, not yet implemented)

The component rows above describe the implemented surface plus the `Daemon.Cluster.Profile`,
`Daemon.Test.Config.*`, and `Daemon.Test.Integration.*` modules that Phase 9 adds. Phase 9 is
`Blocked` on the upstream `hostbootstrap-core` phases; none of the following is implemented yet:

- **`hostbootstrap-core` dependency** — `hostbootstrap` becomes a Haskell `hostbootstrap-core`
  library plus a thin Python bootstrapper. `daemon-substrate` consumes `hostbootstrap-core` as a
  pinned `source-repository-package` git dependency, adds `optparse-applicative`, and renames the
  project binary; the binary extends the core command tree via `runHostBootstrapCLI`.
- **Three-tier / binary-generated Dhall** — the skeletal `hostbootstrap.dhall` carries only
  `project`, `dockerfile`, and `resources {cpu, memory, storage}`; the rich project-level Dhall
  (roles + cluster bootstrap) and per-case test Dhall are generated by the binary, which emits its
  own schema (`config schema` / `config render`).
- **`ClusterProfile`** — `ProductionProfile` (fixed name under `.data/`) vs `TestProfile <case>`
  (`dst-test-<model>-<archetype>` under `.test_data/<case>/`), with one centralized cluster-name /
  `hostPath` derivation replacing the duplicate logic in `Daemon.Cluster.Runner` and
  `Daemon.Cluster.Kind`.
- **Resource budget** — the per-project Colima VM on Apple (sized to the declared budget) and kind
  node cordoning on Linux are owned by `hostbootstrap-core` and honored per case via a recursive
  `hostbootstrap` invocation.
- **`Daemon.Test.Integration.*`** — the executable per-case 3x3 runner and archetype assertions
  with guaranteed `finally` teardown and a `dst-test-` delete-guard.

See [phase-9-hostbootstrap-core-integration-and-host-driven-3x3.md](phase-9-hostbootstrap-core-integration-and-host-driven-3x3.md).
